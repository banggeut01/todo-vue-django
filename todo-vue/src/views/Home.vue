...
export default {
  ...
  data() {
    // 컴포넌트에서는 반드시 data를 함수로 정의하고, object를 리턴한다.
    return {
      todos: [],
    }
  },
  ...
  methods: {
    todoCreate(title) {
      ...
      axios.post('http://127.0.0.1:8000/api/v1/todos/', data, this.options) // this.options는 computed 변수
        .then(response => {
